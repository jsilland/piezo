...
public abstract class EnvelopeClientHandler<I, O> extends SimpleChannelInboundHandler<O> {
   ...
   *
   * @param outputParser a parser for the response's protobuf type
   * @param <O> the type of the RPC response
   */
  public <O extends Message> EnvelopeFuture<O> newProvisionalResponse(Parser<O> outputParser) {
    long requestId = ThreadLocalRandom.current().nextLong();
    EnvelopeFuture<O> outputFuture = new EnvelopeFuture<>(requestId, new Cancel(requestId),
        outputParser, clientLogger);
